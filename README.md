...
The 3COM 3C509B card is advertised to work with computers with a 16-bit ISA bus, but the reason for this is not that the card physically requires it, but because the software uses assembly opcodes found in the 286 processor. That means the original IBM PC and XT (which have an 8-bit ISA bus) cannot use the card. Or rather, they couldn't.
